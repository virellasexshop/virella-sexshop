...
export default async function AdminOrdersPage() {
  ...
  return <main className="adminShell"><AdminSidebar /><section className="adminContent">
    ...
    {orders.length ? <div className={styles.list}>{orders.map((order) => <article className={styles.card} key={order.id}>
      ...
      <div className={styles.body}><div><h3>Itens</h3><ul>{(order.pedido_itens || []).map((item, index) => <li key={`${item.nome}-${index}`}>{item.quantidade}× {item.nome}</li>)}</ul></div><div><h3>Entrega</h3><p><strong className={styles.shippingService}>{order.frete_transportadora || "Transportadora"} — {order.frete_servico_nome || "Entrega"}</strong><br />{order.frete_gratis ? "Frete grátis" : money(order.frete)}{order.frete_prazo_dias ? ` · até ${order.frete_prazo_dias} dias úteis` : ""}<br /><br />{order.endereco_rua}, {order.endereco_numero}{order.endereco_complemento ? ` — ${order.endereco_complemento}` : ""}<br />{order.endereco_bairro} — {order.endereco_cidade}/{order.endereco_estado}<br />CEP {order.endereco_cep}</p></div><form action={updateOrderStatusAction}><input type="hidden" name="pedido_id" value={order.id} /><label>Andamento<select name="status_pedido" defaultValue={order.status_pedido}><option value="novo">Pedido recebido</option><option value="em_separacao">Em separação</option><option value="enviado">Enviado</option><option value="entregue">Entregue</option><option value="cancelado">Cancelado</option></select></label><button type="submit">Atualizar</button></form></div>

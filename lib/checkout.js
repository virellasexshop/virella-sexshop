...
export function validateCustomer(rawCustomer = {}) {
  const customer = {
    ...
    documento: cleanText(rawCustomer.documento, 18).replace(/\D/g, ""),
  ...
  if (![11, 14].includes(customer.documento.length)) throw new Error("Informe um CPF ou CNPJ válido para a entrega.");
...
export async function calculateCheckout(rawItems) {
  ...
  return { items, subtotal, frete: 0, total: subtotal };
}

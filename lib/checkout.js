...
function normalizeCart(rawItems) {
  ...
  for (const item of rawItems.slice(0, MAX_DIFFERENT_PRODUCTS)) {
    ...
    const current = quantities.get(key) || { id, variacao_id: variationId, quantidade: 0 };
...
export async function calculateCheckout(rawItems) {
  ...
  const items = cart.map((cartItem) => {
    ...
    const available = Number(variation ? variation.quantidade : product.quantidade);
    if (Number.isFinite(available) && available >= 0 && cartItem.quantidade > available) {
      throw new Error(`Estoque insuficiente para ${product.nome}${variation ? ` — ${variation.nome}` : ""}.`);
    }
    ...
    return {
      ...
      nome: variation ? `${product.nome} — ${variation.nome}` : product.nome,

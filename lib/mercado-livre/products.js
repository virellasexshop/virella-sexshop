...
function mapCommonAttributes(product, input) {
  const attrs = [];
  ...
  if (gtin) {
    ...
    attrs.push({ id: "EMPTY_GTIN_REASON", value_name: emptyGtinReason });
  }
...
function buildVariations(product, input, variationAttributeId) {
...
export async function publishProductToMercadoLivre(productId, input = {}) {
  ...
  if (gtinMeta?.tags?.required && !gtin && !emptyGtinReason) {
  ...
  const variationAttributeId = allowedVariationIds.has(requestedVariationAttribute)
    ...
    : null;
  const variations = buildVariations(product, input, variationAttributeId);
  ...
  const body = {
    ...
    pictures,
    attributes: mapCommonAttributes(product, input),

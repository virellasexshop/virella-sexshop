...
export async function publishProductToMercadoLivre(productId, input = {}) {
  ...
  const emptyGtinReason = cleanText(input.empty_gtin_reason, 255);
  const emptyGtinReasonMeta = findCategoryAttribute(categoryAttributes, "EMPTY_GTIN_REASON");
  const gtinRequired = Boolean(gtinMeta?.tags?.required);
  const gtinConditionalRequired = Boolean(gtinMeta?.tags?.conditional_required);
  // IMPORTANTE: EMPTY_GTIN_REASON só é aceito como substituto seguro quando
  // a categoria o marca explicitamente como required. Há categorias em que
  // GTIN e EMPTY_GTIN_REASON vêm como conditional_required, mas /items ainda
  // rejeita a publicação sem GTIN. Nesse caso exigimos o código real.
  const canUseEmptyGtinReason = Boolean(
    !gtinRequired &&
    emptyGtinReasonMeta?.tags?.required
  );

  if ((gtinRequired || gtinConditionalRequired) && !gtin) {
    if (!canUseEmptyGtinReason) {
      throw new Error("Esta categoria do Mercado Livre exige um GTIN/EAN real para publicar. A opção de motivo sem GTIN não é aceita nesta categoria. Informe o código de barras real da embalagem/fornecedor ou escolha outra categoria correta para o produto.");
    }
    if (!emptyGtinReason) {
      throw new Error("Esta categoria exige GTIN/EAN ou um motivo de ausência aceito pelo Mercado Livre.");
    }
  }

  if (!gtin && emptyGtinReason && !canUseEmptyGtinReason) {
    throw new Error("O Mercado Livre não aceita publicação sem GTIN nesta categoria. Informe um GTIN/EAN válido ou revise a categoria.");
  }

  if (!gtin && emptyGtinReason && canUseEmptyGtinReason && Array.isArray(emptyGtinReasonMeta?.values) && emptyGtinReasonMeta.values.length) {

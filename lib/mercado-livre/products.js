...
export async function publishProductToMercadoLivre(productId, input = {}) {
  ...
  const canUseEmptyGtinReason = Boolean(
    !gtinRequired &&
    emptyGtinReasonMeta &&
    (emptyGtinReasonMeta?.tags?.required || emptyGtinReasonMeta?.tags?.conditional_required)
  );

  if ((gtinRequired || gtinConditionalRequired) && !gtin) {
    if (!canUseEmptyGtinReason) {
      throw new Error("O Mercado Livre exige um GTIN/EAN real nesta categoria. O motivo de ausência do GTIN não é aceito como substituto. Informe o código da embalagem/fornecedor ou revise a categoria.");
    }
    if (!emptyGtinReason) {

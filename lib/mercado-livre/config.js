...
export function getMercadoLivreConfig() {
  ...
  const siteUrl = String(
    process.env.NEXT_PUBLIC_SITE_URL || process.env.SITE_URL || "https://www.virellasexshop.com.br"
  ).replace(/\/$/, "");
  const redirectUri =
  ...
  if (!clientId || !clientSecret) {
    throw new Error(
      "Configure MERCADO_LIVRE_CLIENT_ID e MERCADO_LIVRE_CLIENT_SECRET na Vercel."
    );
  }
